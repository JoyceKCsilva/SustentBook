...
// Verifica se a tabela 'sebo_desejos' existe antes de executar a query
$tableCheck = $pdo->query("SHOW TABLES LIKE 'sebo_desejos'")->rowCount();
if ($tableCheck == 0) {
    die("Erro: A tabela 'sebo_desejos' não existe no banco de dados.");
}
...
<div class="wrapper">
    <aside id="sidebar">     
        <div class="data-bs-target">
            ...
            <ul class="sidebar-nav">
                <li class="sidebar-item"><a href="../index.php" class="sidebar-link"><i class="bi bi-house"></i><span>Início</span></a></li>
                <li class="sidebar-item"><a href="meuperfil.php" class="sidebar-link"><i class="lni lni-user"></i><span>Perfil</span></a></li>
                <li class="sidebar-item"><a href="buscar.php" class="sidebar-link"><i class="lni lni-search-alt"></i><span>Procurar</span></a></li>
                <li class="sidebar-item"><a href="meuslivros.php" class="sidebar-link"><i class="lni lni-book"></i><span>Meus Livros</span></a></li>
                <li class="sidebar-item"><a href="listadesejos.php" class="sidebar-link"><i class="lni lni-list"></i><span>Lista de Desejos</span></a></li>
                <div class="sidebar-footer">
                    <a href="logout.php" class="sidebar-link"><i class="lni lni-exit"></i><span>Sair</span></a>
                </div>
            </ul>
        </div>
    </aside>
    ...
    <div class="main p-3">
        <div class="containergeral">
            <h1>Adicionar Livro à Lista</h1>
            <form action="add-desejo.php" method="POST">
                <div class="form-floating mb-3">
                    <input type="text" class="form-control" id="DSJ_TITULO" name="DSJ_TITULO" placeholder="Título do livro" required>
                    <label for="DSJ_TITULO">Título do livro</label>
                </div>
                <div class="form-floating mb-3">
                    <input type="text" class="form-control" id="DSJ_AUTOR" name="DSJ_AUTOR" placeholder="Autor do livro" required>
                            ...
                            <ul>
                                <?php foreach ($livros_venda[$desejo['DSJ_TITULO']] as $livro): ?>
                                    <li>
                                        <?php 
                                        $seller_id = $livro['LVR_ID_USUARIO']; 
                                        $seller_data = $user_data[$desejo['DSJ_TITULO']][$seller_id] ?? null;
                                        ?>
                                        <?php if ($seller_data): ?>
                                            <img src="<?= htmlspecialchars($seller_data['USR_FOTO']); ?>" alt="Foto do vendedor" width="30" height="30">
                                            <a href="detalheslivro.php?id=<?= $livro['LVR_ID']; ?>">
                                                <?= htmlspecialchars($seller_data['USR_EMAIL']); ?> está vendendo este livro por R$ <?= htmlspecialchars($livro['LVR_PRECO']); ?>
                                            </a>
                                        <?php else: ?>
